...
def postGithubPrComment(def file) {
    ...
    withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'github', usernameVariable: 'GITHUB_USER', passwordVariable: 'GITHUB_TOKEN']]) {
        sh "curl -v -H \"Authorization: token ${GITHUB_TOKEN}\" -X POST -H \"Content-type: application/json\" -d \"@${file}\" \"https://api.github.com/repos/Carrefour-Group/strimzi-kafka-operator/issues/${env.ghprbPullId}/comments\" > out.log 2> out.err"
